...
The console script starts with defaults. To customize, call `run_server()`:
...
Pending messages are persisted through `RendezvousStore` (backed by `json-database`),
so the relay survives restarts. Each message carries an expiry, and expired messages
...
TLS in front of it (a reverse proxy such as nginx or Caddy) instead of exposing
port 6789 directly. Message payloads are already end-to-end encrypted to the
recipient pubkey. TLS also protects the proof exchange and metadata in
transit.
...
---
[← HTTP API](./http-api.md) · [Home](index.md) · [Examples →](examples.md)
