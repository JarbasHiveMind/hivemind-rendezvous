...
1. **Deposit.** Node A serialises a HiveMind `INTERCOM` message, encrypts it to
...
There is no account or session. To retrieve, a node proves it controls the
private key for the pubkey it is claiming:
...
`sign_ownership()` produces the proof on the client; `verify_ownership()` checks it
on the relay (see [HTTP API](http-api.md)).
...
`SHA256(target_pubkey)` — so a depositor cannot file a message encrypted for a
different key into someone else's mailbox.

## Abuse controls

- **Per-IP rate limiting** on `/deposit` (sliding window; defaults 60 deposits per
  60 s).
